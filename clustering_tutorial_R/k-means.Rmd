...
K-means clustering is a type of unsupervised learning exclusive clustering where data can only belong to one cluster. It is one of the most popular/useful clustering algorithms in bioinformatics apart from hierarchical clustering.
...
plot(x = dat$BP_condition_1, y = dat$BP_condition_2, col = "pink", main = "Scatter plot of blood pressure measures", xlab = "BP under condition 1", ylab = "BP under condition 2")
```


From the above graph, we can discern 3 clusters. We will input this number, K, in the k-means function. Make sure K < N.
...
# Clustering Ratio of between SS to total SS - check if ratio is big
