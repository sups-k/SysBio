...
gene_attribute_table = read.table("~/Downloads/R_Prog/SysBio/sysbio_assignment-3/gene_attribute_edges.txt", sep = "\t", header = T)
...
gene_TF_table = data.frame(GENE = gene_attribute_table$source, TF = gene_attribute_table$target)
...
g_gene_TF = graph_from_data_frame(d=gene_TF_table, directed = T)
E(g_gene_TF)$weight = gene_attribute_table$weight
E(g_TF_gene)$weight = gene_attribute_table$weight

deg = degree(g_TF_gene)
deg = degree(g_gene_TF)
