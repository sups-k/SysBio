...
##  If vertices=NULL, then first 2 columns of data frame d are taken as edge lists, and
#   any additional columns as edge attributes.

## If vertices=df, where df is a data frame, then the first column of df is taken as symbolic
#  verted names. These names will go as verted names of graph.

##  If this happens, then, first 2 columns of d are connected in graph as edges, and any name
#   in first column of df not connected in d will be shown as unconnected nodes in the graph.
#   Also, symbolic links of d should contain only the verted names listed in df.
##---------------------------------------------------------------------------
## include the libraries
library("igraph")
library("igraphdata")

proteins <- read.csv("~/Downloads/R_Prog/SysBio/igraph_dummy_analysis/protein_list.csv", header=TRUE)

interactions <- read.csv("~/Downloads/R_Prog/SysBio/igraph_dummy_analysis/protein_interactions.csv")
